...
export default function AdminLayout() {
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-[#090d16] text-slate-100 font-sans">

      {/* Sidebar Panel - Offcanvas on mobile */}
      {sidebarOpen && (
        <div className="md:hidden fixed inset-0 bg-black/50 z-40" onClick={() => setSidebarOpen(false)} />
      )}
      <aside className={`fixed md:relative top-0 bottom-0 left-0 z-50 w-[240px] sm:w-64 bg-slate-950 border-r border-slate-900 flex flex-col justify-between transition-transform duration-300 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0 md:w-20'}`}>
        ...
        {/* Footer logout */}
        <div className="flex-shrink-0 p-3 sm:p-4 border-t border-slate-900 pb-20 md:pb-4">
          <button
