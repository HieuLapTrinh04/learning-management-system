...
export default function LandingPage() {
  ...
  return (
    <div className="w-full flex flex-col bg-[#020617] text-slate-100 overflow-hidden font-sans selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      ========================================== */}
      <section className="relative w-full min-h-[90vh] flex items-center pt-10 pb-16 md:pt-20 md:pb-32 overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-8">
            ...
            <div className="flex-1 space-y-6 md:space-y-8 text-center lg:text-left z-20">
              ...
              <RevealOnScroll delay={0.5}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 pt-8 md:pt-12 border-t border-slate-800/50 mt-8 md:mt-12">
                  <div>
                    <div className="text-xl md:text-2xl font-bold text-white mb-1">50K+</div>
                    <div className="text-[10px] md:text-xs text-slate-500 font-medium">Học viên</div>
                  </div>
                  <div>
                    <div className="text-xl md:text-2xl font-bold text-white mb-1">1K+</div>
                    <div className="text-[10px] md:text-xs text-slate-500 font-medium">Khóa học</div>
                  </div>
                  <div>
                    <div className="text-xl md:text-2xl font-bold text-white mb-1">200+</div>
                    <div className="text-[10px] md:text-xs text-slate-500 font-medium">Giảng viên</div>
                  </div>
                  <div>
                    <div className="flex items-center justify-center lg:justify-start gap-1 text-xl md:text-2xl font-bold text-white mb-1">
                      4.9 <Star className="w-3 h-3 md:w-4 md:h-4 fill-amber-500 text-amber-500" />
                    </div>
                    <div className="text-[10px] md:text-xs text-slate-500 font-medium">Đánh giá</div>
                  </div>
                </div>
              </RevealOnScroll>
            </div>
      ...
      ========================================== */}
      <section className="py-12 md:py-24 relative z-20 bg-[#020617]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <RevealOnScroll delay={0.1}>
            <div className="text-center mb-8 md:mb-16">
              <h2 className="text-2xl md:text-4xl font-bold text-white mb-2 md:mb-4">Khám Phá Các Danh Mục</h2>
              <p className="text-xs md:text-base text-slate-400">Khám phá các kỹ năng đang có nhu cầu cao trên mọi lĩnh vực.</p>
            </div>
          </RevealOnScroll>

          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 md:gap-4">
            {[
              { title: "Lập trình", icon: <MonitorPlay className="w-6 h-6 md:w-8 md:h-8 text-blue-400" />, color: "from-blue-500/20 to-cyan-500/5", count: "120+ Khóa học" },
              { title: "Dữ liệu", icon: <Database className="w-6 h-6 md:w-8 md:h-8 text-emerald-400" />, color: "from-emerald-500/20 to-teal-500/5", count: "85+ Khóa học" },
              { title: "Kinh doanh", icon: <Briefcase className="w-6 h-6 md:w-8 md:h-8 text-amber-400" />, color: "from-amber-500/20 to-orange-500/5", count: "200+ Khóa học" },
              { title: "Thiết kế UI/UX", icon: <LayoutDashboard className="w-6 h-6 md:w-8 md:h-8 text-pink-400" />, color: "from-pink-500/20 to-rose-500/5", count: "60+ Khóa học" },
              { title: "Ngoại ngữ", icon: <MessageCircle className="w-6 h-6 md:w-8 md:h-8 text-purple-400" />, color: "from-purple-500/20 to-indigo-500/5", count: "150+ Khóa học" },
              { title: "Marketing", icon: <Zap className="w-6 h-6 md:w-8 md:h-8 text-yellow-400" />, color: "from-yellow-500/20 to-amber-500/5", count: "90+ Khóa học" }
            ].map((cat, idx) => (
              <RevealOnScroll key={idx} delay={0.1 * idx}>
                <div className={`bg-gradient-to-b ${cat.color} border border-slate-800 hover:border-slate-600 p-4 md:p-6 rounded-2xl md:rounded-3xl backdrop-blur-sm transition-all duration-300 hover:-translate-y-2 cursor-pointer group h-full`}>
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-slate-900/80 shadow-inner border border-white/5 flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 transition-transform">
                    {cat.icon}
                  </div>
                  <h3 className="text-sm md:text-base font-bold text-white mb-1">{cat.title}</h3>
                  <p className="text-[10px] md:text-xs text-slate-500 font-medium">{cat.count}</p>
                </div>
              </RevealOnScroll>
            ))}
          </div>
      ...
      ========================================== */}
      <section className="py-12 md:py-24 bg-[#050b1a] border-y border-slate-800/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <RevealOnScroll delay={0.1}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-12 gap-4 md:gap-4">
              <div>
                <h2 className="text-2xl md:text-4xl font-bold text-white mb-2 md:mb-4">Các Khóa Học Phổ Biến Nhất</h2>
                <p className="text-xs md:text-sm text-slate-400">Tham gia cùng hàng ngàn học viên trong các chương trình được đánh giá cao này.</p>
              </div>
              <Link to="/courses" className="px-4 py-2 md:px-6 md:py-3 bg-slate-900 border border-slate-700 hover:bg-slate-800 text-white text-xs md:text-sm font-semibold rounded-full flex items-center gap-2 transition w-full md:w-auto justify-center md:justify-start mt-2 md:mt-0">
                Xem tất cả khóa học
              </Link>
            </div>
          </RevealOnScroll>
          ...
          {isLoading ? (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
              {[1, 2, 3, 4].map(i => <div key={i} className="h-48 md:h-96 bg-slate-800/50 animate-pulse rounded-2xl md:rounded-3xl"></div>)}
            </div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                  ...
                  <Link to={`/courses/${course.slug || course.id}`} className="group bg-[#020617] border border-slate-800 rounded-2xl md:rounded-3xl overflow-hidden hover:border-indigo-500/50 transition-all duration-300 hover:shadow-[0_20px_40px_rgba(99,102,241,0.1)] hover:-translate-y-1 flex flex-col h-full">
                    ...
                    <div className="p-3 md:p-5 flex flex-col flex-1">
                      <div className="flex flex-col md:flex-row md:items-center gap-1.5 md:gap-2 mb-2">
                        ...
                        <div className="flex items-center gap-1 text-[9px] md:text-[11px] text-amber-400 font-bold">
                      ...
                      <div className="mt-auto">
                        ...
                        <div className="flex items-center justify-between border-t border-slate-800/80 pt-2.5 md:pt-4">
                          ...
                          <div className="w-5 h-5 md:w-8 md:h-8 rounded-full bg-slate-800 group-hover:bg-indigo-500 flex items-center justify-center transition-colors">
                            <ArrowRight className="w-2.5 h-2.5 md:w-4 md:h-4 text-slate-300 group-hover:text-white" />
                          </div>
                        </div>
                      </div>
                    </div>
      ...
      ========================================== */}
      <section className="py-12 md:py-24 bg-[#020617] relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <RevealOnScroll delay={0.1}>
            <div className="text-center mb-8 md:mb-16 max-w-3xl mx-auto">
              <h2 className="text-2xl md:text-5xl font-bold text-white mb-4 md:mb-6">Thiết kế cho thành công của bạn</h2>
              <p className="text-sm md:text-lg text-slate-400">Mọi thứ bạn cần để làm chủ kỹ năng mới và thăng tiến sự nghiệp, được thiết kế với trải nghiệm học tập cao cấp.</p>
            </div>
          </RevealOnScroll>
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
            {[
              ...
              <RevealOnScroll key={idx} delay={0.1 * idx}>
                <div className="relative group p-4 md:p-8 bg-slate-900/50 border border-slate-800 rounded-2xl md:rounded-3xl overflow-hidden hover:bg-slate-800/50 transition-colors h-full">
                  <div className="absolute top-0 right-0 w-20 h-20 md:w-32 md:h-32 bg-gradient-to-bl from-indigo-500/10 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                  <div className="w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 flex items-center justify-center mb-3 md:mb-6 text-indigo-400 shadow-lg group-hover:scale-110 transition-transform duration-300">
                    {React.cloneElement(feature.icon, { className: "w-5 h-5 md:w-8 md:h-8" })}
                  </div>
                  <h3 className="text-sm md:text-xl font-bold text-white mb-2 md:mb-3">{feature.title}</h3>
                  <p className="text-slate-400 text-[10px] md:text-sm leading-relaxed line-clamp-3 md:line-clamp-none">{feature.desc}</p>
                </div>
              </RevealOnScroll>
            ))}
          </div>
      ...
      ========================================== */}
      <section className="py-12 md:py-24 bg-[#050b1a] border-y border-slate-800/50 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <RevealOnScroll delay={0.1}>
            <div className="text-center mb-8 md:mb-20">
              <h2 className="text-2xl md:text-4xl font-bold text-white mb-2 md:mb-4">Lộ Trình Làm Chủ Kỹ Năng</h2>
              <p className="text-xs md:text-sm text-slate-400 max-w-xl mx-auto">Một phương pháp đã được chứng minh để đưa bạn từ người mới bắt đầu đến sẵn sàng làm việc.</p>
            </div>
          </RevealOnScroll>
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-6 relative z-10">
              {[
                ...
                <RevealOnScroll key={idx} delay={0.3 + (idx * 0.2)}>
                  <div className="relative flex flex-col items-center text-center lg:items-start lg:text-left h-full">
                    <div className="w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-slate-900 border-2 border-indigo-500 flex items-center justify-center font-bold text-sm md:text-xl text-white mb-3 md:mb-6 shadow-[0_0_20px_rgba(99,102,241,0.2)] md:shadow-[0_0_30px_rgba(99,102,241,0.3)]">
                      {item.step}
                    </div>
      ...
      ========================================== */}
      <section className="py-12 md:py-24 bg-[#020617]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <RevealOnScroll delay={0.1}>
            <div className="mb-8 md:mb-16 text-center">
              <h2 className="text-2xl md:text-5xl font-bold text-white mb-3 md:mb-6 leading-tight">Đừng chỉ nghe những gì chúng tôi nói</h2>
              <p className="text-sm md:text-lg text-slate-400">Hàng ngàn chuyên gia đã thay đổi sự nghiệp của họ nhờ nền tảng của chúng tôi.</p>
            </div>
          </RevealOnScroll>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
            {testimonials.map((review, idx) => (
              <RevealOnScroll key={review.id || idx} delay={0.2 * idx}>
                <div className="bg-slate-900/40 border border-slate-800 p-5 md:p-8 rounded-2xl md:rounded-3xl relative h-full flex flex-col">
                  <Quote className="absolute top-4 right-4 w-6 h-6 md:top-6 md:right-6 md:w-12 md:h-12 text-slate-800/50" />
                  <div className="flex text-amber-400 mb-4 md:mb-6">
                    {[...Array(review.rating || 5)].map((_, i) => (
                      <Star key={i} className="w-3.5 h-3.5 md:w-4 md:h-4 fill-amber-400" />
                    ))}
                  </div>
                  <p className="text-[13px] md:text-base text-slate-300 leading-relaxed mb-5 md:mb-8 relative z-10 flex-1">"{review.content}"</p>
                  <div className="flex items-center gap-3 md:gap-4 border-t border-slate-800/50 pt-4 md:pt-6 mt-auto">
                    <img src={review.avatar_url || 'https://via.placeholder.com/150'} alt={review.name} className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-slate-700 object-cover" />
                    <div>
                      <h4 className="font-bold text-sm md:text-base text-white">{review.name}</h4>
                      <p className="text-[10px] md:text-xs text-indigo-400 font-medium">{review.role}</p>
                    </div>
                  </div>
                </div>
              </RevealOnScroll>
      ...
      ========================================== */}
      <section className="py-12 md:py-24 bg-gradient-to-b from-[#050b1a] to-[#020617] border-t border-slate-800/50 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row items-center gap-10 md:gap-16">
            <div className="flex-1 space-y-5 md:space-y-8">
              <RevealOnScroll delay={0.1}>
                <h2 className="text-2xl md:text-5xl font-bold text-white text-center lg:text-left leading-tight">Chứng minh kỹ năng với Chứng Nhận Uy Tín</h2>
              </RevealOnScroll>
              <RevealOnScroll delay={0.2}>
                <p className="text-sm md:text-lg text-slate-400 leading-relaxed text-center lg:text-left">
              ...
              <RevealOnScroll delay={0.3}>
                <ul className="space-y-3 md:space-y-4">
                  <li className="flex items-center gap-2 md:gap-3 text-xs md:text-base text-slate-300">
                    <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 text-emerald-400 flex-shrink-0" /> URL xác minh duy nhất cho mỗi chứng chỉ
                  </li>
                  <li className="flex items-center gap-2 md:gap-3 text-xs md:text-base text-slate-300">
                    <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 text-emerald-400 flex-shrink-0" /> Bảo mật và xác thực bằng công nghệ Blockchain
                  </li>
                  <li className="flex items-center gap-2 md:gap-3 text-xs md:text-base text-slate-300">
                    <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 text-emerald-400 flex-shrink-0" /> Tích hợp LinkedIn chỉ với 1 click
                  </li>
                </ul>
              </RevealOnScroll>
              <RevealOnScroll delay={0.4}>
                <div className="text-center lg:text-left">
                  <Link to="/courses" className="inline-flex items-center gap-2 px-5 py-2.5 md:px-6 md:py-3 bg-slate-800 hover:bg-slate-700 text-white text-sm md:text-base font-bold rounded-xl transition">
            ...
            <div className="flex-1 relative w-full aspect-[4/3] perspective-[1000px] mt-8 lg:mt-0">
              <RevealOnScroll delay={0.3} className="h-full w-full">
                ...
                <div className="absolute inset-0 transform-style-3d rotate-y-[-10deg] rotate-x-[5deg] hover:rotate-y-[0deg] hover:rotate-x-[0deg] transition-transform duration-700">
                  <div className="w-[120%] h-[120%] -ml-[10%] -mt-[10%] bg-gradient-to-br from-amber-500/20 to-amber-900/10 blur-[60px] md:blur-[80px] rounded-full absolute z-0"></div>
                  <div className="w-full h-full bg-slate-100 rounded-lg p-1.5 md:p-2 shadow-[0_10px_30px_rgba(0,0,0,0.5)] md:shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/20 relative z-10 flex flex-col">
                    <div className="flex-1 border-[3px] md:border-[4px] border-double border-slate-300 flex flex-col items-center justify-center p-4 md:p-8 text-center relative bg-white">
                      <div className="absolute top-2 left-2 md:top-4 md:left-4 w-8 h-8 md:w-12 md:h-12 bg-slate-900 flex items-center justify-center rounded">
                        <Award className="w-4 h-4 md:w-6 md:h-6 text-amber-500" />
                      </div>
                      <h3 className="text-[9px] md:text-sm font-bold text-slate-400 uppercase tracking-widest mb-2 md:mb-4 mt-4 md:mt-0">Chứng Chỉ Hoàn Thành</h3>
                      <h2 className="text-xl md:text-4xl font-serif text-slate-900 mb-3 md:mb-6">Lập Trình React Nâng Cao</h2>
                      ...
                      <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8">
                        <div className="w-10 h-10 md:w-16 md:h-16 bg-amber-50 rounded-full border border-amber-200 flex items-center justify-center">
                          <ShieldCheck className="w-5 h-5 md:w-8 md:h-8 text-amber-500" />
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      ========================================== */}
      <section className="py-12 md:py-24 bg-[#020617] border-t border-slate-800/50">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <RevealOnScroll delay={0.1}>
            <div className="text-center mb-8 md:mb-16">
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-2 md:mb-4">Câu Hỏi Thường Gặp</h2>
              <p className="text-sm md:text-base text-slate-400">Mọi thông tin bạn cần biết về nền tảng và cách thức hoạt động.</p>
            </div>
          </RevealOnScroll>
            ...
            {faqs.map((faq, idx) => (
              <RevealOnScroll key={idx} delay={0.1 * idx}>
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden">
                  ...
                  >
                    <span className="text-[13px] md:text-base font-semibold text-white pr-4">{faq.q}</span>
                    {activeFaq === idx ? <Minus className="w-4 h-4 md:w-5 md:h-5 text-indigo-400 flex-shrink-0" /> : <Plus className="w-4 h-4 md:w-5 md:h-5 text-slate-500 flex-shrink-0" />}
                  </button>
                  <div className={`px-4 md:px-6 overflow-hidden transition-all duration-300 ease-in-out ${activeFaq === idx ? 'max-h-40 pb-3 md:pb-5 opacity-100' : 'max-h-0 opacity-0'}`}>
                    <p className="text-xs md:text-base text-slate-400 leading-relaxed">{faq.a}</p>
                  </div>
                </div>
              </RevealOnScroll>
            ))}
      ...
      <section className="py-12 md:py-24 relative overflow-hidden bg-[#050b1a]">
        ...
        <div className="max-w-4xl mx-auto px-4 relative z-10 text-center space-y-5 md:space-y-8">
          ...
          <RevealOnScroll delay={0.2}>
            <h2 className="text-2xl md:text-6xl font-bold text-white tracking-tight">
          ...
          <RevealOnScroll delay={0.4}>
            ...
            <p className="text-[10px] md:text-xs text-slate-500 mt-4 md:mt-6">Hủy bất cứ lúc nào. Hoàn tiền 100% trong 7 ngày đối với khóa học trả phí.</p>
          </RevealOnScroll>
        </div>
      </section>
